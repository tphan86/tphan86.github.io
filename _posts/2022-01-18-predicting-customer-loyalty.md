...
Our client, a food retailer, hired a market research consultancy to add market-level customer loyalty information to the database. However, only about 50 % of the client's customer base could be labelled; thus, the other half did not have this information.   Let's use ML to solve this!
...
Our client, a food retailer, hired a market research consultancy to add market-level customer loyalty information to the database. However, only about 50 % of the client's customer base could be labelled; thus, the other half did not have this information. 

The general goal of this work is to accurately predict the loyalty score of customers who are unable to be classified, to enable our customers to clearly understand the true loyalty of customers regardless of total spending and to enable more accurate and relevant tracking, targeting and communication with customers.

To do so, we are looking to create a predictive model that determines the relationship between customer metrics and the loyalty score of tagged customers and uses it to predict loyalty score metrics of non-tagged customers. 

<br>
<br>
### Actions <a name="overview-actions"></a>

We firstly needed to compile the necessary data from tables in the database, gathering key customer metrics that may help predict *loyalty score*, appending to the dependent variable, and separating out those who did and did not have this dependent variable present.
...
As the most important outcome for this project was predictive accuracy, rather than explicitly understanding weighted prediction drivers, we chose Random Forest as the model to use for making predictions on customers who were missing the *loyalty score* metric.
<br>
<br>
### Growth/Next Steps <a name="overview-growth"></a>

Although prediction accuracy was relatively high, other modeling approaches, such as XGBoost and LightGBM, could be tested to find out whether further accuracy is achieved, especially for approaches similar to random forests. 

From a data point of view, we could collect more variables and perform more feature engineering to ensure that we have as much useful information as possible to predict client loyalty.
